...
const { globalPool } = require('../config/db');
...
DB_PORT=5432
    ...
    const insertPropertiesQuery = `
        INSERT INTO test (property_id, property_name, address, contact_number, email, business_hours, tax_reg_no, state, district, country, currency, is_saved, created_at, updated_at, status   )
...
router.post('/approve/:id', async (req, res) => {
  ...
  try {
    ...
    const subdomain = `${cleanName}.example.com`; // Subdomain format
